...
void make_dlist(FT_Face face, char ch, GLuint list_base, GLuint * tex_base, unsigned int h) {
	...
	glTranslatef(0, (GLfloat)(h - bitmap_glyph->top), 0);
...
void font_data::init(const char * fname, unsigned int h) {
	...
	for (unsigned char i = 0; i<128; i++)
		make_dlist(face, i, list_base, textures, h);
...
void drawText(const font_data &ft_font, float x, float y, float scalex, float scaley, const char *string, ...)  {
	...
	float h = ft_font.h;						//We make the height about 1.5* that of
	...
	if (string == NULL)									// If There's No Text
		*text = 0;											// Do Nothing
		...
		va_start(args, string);									// Parses The String For Variables
		vsprintf_s(text, string, args);						// And Converts Symbols To Actual Numbers
	...
	for (unsigned int i = 0; i < str.length(); i++)
	{
		if (str.at(i) == 10)
		{
			lines.push_back(str.substr(position, i - position));
			position = i + 1;
		}
		if (i == str.length() - 1)
		{
			lines.push_back(str.substr(position, i - position + 1));
			position = i + 1;
		}
	}
